/****************************************************************************
** Meta object code from reading C++ file 'ccSubsamplingDlg.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.7.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../CloudCompare-master/qCC/ccSubsamplingDlg.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'ccSubsamplingDlg.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.7.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_ccSubsamplingDlg_t {
    QByteArrayData data[8];
    char stringdata0[93];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_ccSubsamplingDlg_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_ccSubsamplingDlg_t qt_meta_stringdata_ccSubsamplingDlg = {
    {
QT_MOC_LITERAL(0, 0, 16), // "ccSubsamplingDlg"
QT_MOC_LITERAL(1, 17, 11), // "sliderMoved"
QT_MOC_LITERAL(2, 29, 0), // ""
QT_MOC_LITERAL(3, 30, 9), // "sliderPos"
QT_MOC_LITERAL(4, 40, 19), // "samplingRateChanged"
QT_MOC_LITERAL(5, 60, 5), // "value"
QT_MOC_LITERAL(6, 66, 20), // "changeSamplingMethod"
QT_MOC_LITERAL(7, 87, 5) // "index"

    },
    "ccSubsamplingDlg\0sliderMoved\0\0sliderPos\0"
    "samplingRateChanged\0value\0"
    "changeSamplingMethod\0index"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_ccSubsamplingDlg[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   29,    2, 0x09 /* Protected */,
       4,    1,   32,    2, 0x09 /* Protected */,
       6,    1,   35,    2, 0x09 /* Protected */,

 // slots: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Double,    5,
    QMetaType::Void, QMetaType::Int,    7,

       0        // eod
};

void ccSubsamplingDlg::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        ccSubsamplingDlg *_t = static_cast<ccSubsamplingDlg *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->sliderMoved((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->samplingRateChanged((*reinterpret_cast< double(*)>(_a[1]))); break;
        case 2: _t->changeSamplingMethod((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObject ccSubsamplingDlg::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_ccSubsamplingDlg.data,
      qt_meta_data_ccSubsamplingDlg,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *ccSubsamplingDlg::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *ccSubsamplingDlg::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_ccSubsamplingDlg.stringdata0))
        return static_cast<void*>(const_cast< ccSubsamplingDlg*>(this));
    if (!strcmp(_clname, "Ui::SubsamplingDialog"))
        return static_cast< Ui::SubsamplingDialog*>(const_cast< ccSubsamplingDlg*>(this));
    return QDialog::qt_metacast(_clname);
}

int ccSubsamplingDlg::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 3;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
